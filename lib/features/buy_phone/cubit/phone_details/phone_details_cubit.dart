...
class BuyPhoneDetailsCubit extends Cubit<BuyPhoneDetailsState> {
  ...
  BuyPhoneDetailsCubit({required this.repository, required this.favoriteCubit})
  ...
  Future<void> getProductDetails(String id) async {
    ...
    final product = success.data.data!;

    product.isFavorite = favoriteCubit.favorites[id] ?? false;

    emit(BuyPhoneDetailsSuccess(product: product));
  }

  void updateFavorite(bool isFavorite) {
    if (state is BuyPhoneDetailsSuccess) {
      final current = (state as BuyPhoneDetailsSuccess).product;

      current.isFavorite = isFavorite;

      emit(BuyPhoneDetailsSuccess(product: current));
    }
  }
}
